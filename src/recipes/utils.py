...
def get_top_ingredients_chart(recipes):
  ...
  ingredients, counts = zip(*ingredient_count.most_common(5)) if all_ingredients else ([], [])
...
def get_difficulty_spread_chart(recipes):
  ...
  difficulty_spread = Counter(recipe['difficulty'] for recipe in recipes if 'difficulty' in recipe)
  #switch plot backend to AGG (Anti-Grain Geometry) - to write to file
  plt.switch_backend('AGG')
  #specify figure size
